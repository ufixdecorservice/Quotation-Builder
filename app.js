...
/**
 * ==========================================================================
 * NEW: AUTH & API LOGIC (INTEGRATED FROM V2)
 * ==========================================================================
 */
const CONFIG = {
...
async function callAPI(action, payload) {
    const body = { token: CONFIG.API_TOKEN, action: action, payload: payload };
...
async function saveToCloud() {
    ...
    btn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> กำลังบันทึกลง Sheet...';
    btn.disabled = true;

    try {
        const payload = {
            quotationNo: currentData.documentNo,
            date: currentData.dateCreated,
            customerName: currentData.customerName,
            phone: currentData.customerPhone,
            email: currentData.customerEmail,
            address: currentData.customerAddress,
            items: currentData.items,
            subtotal: currentData.subtotal || 0,
            vat: currentData.vat || 0,
            grandTotal: currentData.grandTotal || 0
        };
...
function initApp() {
    ...
    bindBtn('btn-save-cloud', () => saveToCloud());
    bindBtn('btn-add-item-sidebar', () => addItem());
    bindBtn('btn-add-item-preview', () => addItem());
...
function saveDraft() {
    localStorage.setItem('quotation_builder_data', JSON.stringify(currentData));
    alert('บันทึกข้อมูลแบบร่างลงในเบราว์เซอร์สำเร็จแล้ว!');
}
